export function mapAdsErrorMsg(status: number, errorText = ""): string | undefined {
  ...
  if (status === 403 && txt.includes('access_token_scope_insufficient')) {
    return 'Missing Google Ads scope. Re-run: gcloud auth application-default login --scopes=https://www.googleapis.com/auth/adwords';
  }
  // Invalid/expired token
  if (status === 401 || txt.includes('invalid token') || txt.includes('invalid credentials')) {
    return 'Invalid or expired token. Refresh ADC: gcloud auth application-default login --scopes=https://www.googleapis.com/auth/adwords';
  }
